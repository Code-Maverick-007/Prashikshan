...
const DeadlinesWidget = ({ deadlines }) => (
    <div className="bg-surface dark:bg-dark-surface p-6 rounded-xl border border-border dark:border-dark-border">
        ...
        <div className="space-y-3">
            ...
            )) : <p className="text-sm text-text-secondary dark:text-dark-text-secondary text-center py-4">No deadlines to show.</p>}
        </div>
    </div>
);
...
const FeedbackWidget = ({ feedback }) => (
    <div className="bg-surface dark:bg-dark-surface p-6 rounded-xl border border-border dark:border-dark-border">
        ...
        <div className="space-y-4">
            ...
            )) : <p className="text-sm text-text-secondary dark:text-dark-text-secondary text-center py-4">No feedback available yet.</p>}
        </div>
    </div>
);
                ...
                <div className="lg:col-span-2 space-y-6">
                    {applicationsData.length === 0 ? (
                        ...
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                            ...
                            <div className="flex gap-2 bg-surface dark:bg-dark-surface p-1 rounded-lg">{['All', 'Shortlisted', 'Interview', 'Selected'].map(status => (<button key={status} onClick={() => setFilter(status)} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${filter === status ? 'bg-primary text-white' : 'text-text-secondary dark:text-dark-text-secondary hover:text-text dark:hover:text-dark-text'}`}>{status}</button>))}</div>
                        </div>
                        <div className="space-y-4">
